...
1. **你发参考图、告诉我发哪个平台、再给文案。** 参考图是必须的；发哪个平台（小红书 / 公众号 / B 站 / 方图 / 视频…）我会先问清楚，**不会默认帮你按小红书做**——尺寸选错整张白做；文案（也就是你封面上要放的文字）如果暂时没想好，我也会先问你一句，不会替你瞎编。
2. **我分析完，交给你一份"可调版" HTML。** 这是一份可以在浏览器里直接打开的网页文件，左边是封面预览，右边带一个调整面板——这一步先不出最终的图片，是让你先确认效果对不对。
3. **你在浏览器里打开它，自己动手拖改。** 改文字直接在输入框打字、换颜色直接点色板、想挪动标题位置直接用鼠标拖，想换图直接点封面上的图片区域选一张自己的图，想切中文排版还是英文排版直接点面板上的「中文预设 / 英文预设」按钮（切中文会自动放松行高和间距，换语言也不散版），全程不用碰代码。
